...
//!
//! UBI standards for "Universal Binary Installer". It is a tool for downloading and installing
//! pre-built binaries from GitHub releases. It is designed to be used in shell scripts and other
//! automation.
//!
//! This project also includes a CLI tool named `ubi`. See [the project's
//! GitHub repo](https://github.com/houseabsolute/ubi) for more details on
...
impl<'a> UbiBuilder<'a> {
    ...
    /// Set the project to download from. This can either be just the
    /// org/name, like `houseabsolute/precious`, or a complete GitHub URL to
    /// the project, like `https://github.com/houseabsolute/precious`. It also
    ...
    /// Set the tag to download. By default the most recent release is
    /// downloaded. You cannot set this with the `url` option.
    #[must_use]
    ...
    /// Set the URL to download from. This can be provided instead of a
    /// project or tag. This will not use the GitHub API, so you will never
    /// hit the GitHub API limits. That in turn means you won't have to set a
    /// `GITHUB_TOKEN` env var except when downloading a release from a
    /// private repo when the URL is set.
    ///
    ...
    /// Set the directory to install the binary in. If not set it will default
    /// to `./bin`.
    #[must_use]
    ...
    /// Set a string to match against the release filename when there are
    /// multiple files for your OS/arch, i.e. "gnu" or "musl".  Note that this
    /// will be ignored if there is only used when there is only one matching
    /// release filename for your OS/arch
    #[must_use]
    ...
    /// Set the name of the executable to look for in archive files. By
    /// default this is the same as the project name, so for
    /// `houseabsolute/precious` we look for `precious` or
    ...
    /// Set a GitHub token to use for API requests. If this is not set then
    /// the `GITHUB_TOKEN` env var will be checked instead.
    #[must_use]
    ...
    /// Set the platform to download for. If not set it will be determined
    /// based on the current platform's OS/arch.
    #[must_use]
    ...
    /// Set the base URL for the GitHub API. This is useful for testing or if
    /// you want to operate against a GitHub Enterprise installation.
    #[must_use]
    ...
    /// # Errors
    ///
    /// If you have tried to set incompatible options (setting a `project` or
    /// `tag` with a `url`) or you have not set required options (one of
    /// `project` or `url`), then this method will return an error.
...
/// `Ubi` is the core of this library, and is used to download and install a
/// binary.
#[derive(Debug)]
pub struct Ubi<'a> {
...
/// sake of the `ubi` binary, but it lives in the library crate so that test
/// code can also enable logging.
///
/// # Errors
///
